...
describe("Codex UserPromptSubmit I/O", () => {
  test("injects a sanitized polish-only workflow without copying the raw draft", () => {
    ...
    expect(context).toContain("strictly polish-only")
    expect(context).toContain("mcp__dictum__polish_brief")
    expect(context).toContain("mcp__dictum__present_prompt")
    expect(context).toContain('only if present_prompt returns decision "act"')
    expect(context).toContain('For "regenerate"')
    expect(context).toContain('For "tweak", apply the supplied corrections')
    expect(context).toContain('For "feedback_required"')
    expect(context).toContain("Fail closed")
    expect(context).toContain("numbered text fallback")
